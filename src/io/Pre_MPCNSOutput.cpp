...
void OUTPUT_MPCNS::output_inner_file(int32_t my_id)
{
    std::ofstream file;
    //=============================================================================================
    std::string _my_id_s;
    ...
    file << std::endl;
    file << "imin\timax\tjmin\tjmax\tkmin\tkmax\tid\tPhysic_Name\n";
    ...
    for (int32_t iiblock = 0; iiblock < gp->pro_block_index(my_id).size(); iiblock++)
    {
        for (int32_t jjblock = 0; jjblock < gp->pro_block_index(my_id).size(); jjblock++)
        {
            ...
            for (int32_t iface = 0; iface < inner_info.inner.size(); iface++)
            {
                ...
                if (inner_info.inner[iface].is_parallel == 0) // 如果是正常的内部边界条件，没有任何区别
                    file << iiblock + 1;
                else // 如果是周期边界条件产生的内部边界条件，块号变为负值
                    file << -iiblock - 1;

                if (if_multiphy_facecouple) // 如果是多物理场面耦合，严格输出名称
                    file << "\t" << pt->blk(gp->pro_block_index(my_id)[iiblock]).hiera[0].blk_name << std::endl;
                else // 输出"Fluid"
                    file << "\tFluid\n";
                //---------------------------------------------------------------------------------
                ...
                if (if_multiphy_facecouple) // 如果是多物理场面耦合，严格输出名称
                    file << "\t" << pt->blk(gp->pro_block_index(my_id)[jjblock]).hiera[0].blk_name << std::endl;
                else // 输出"Fluid"
                    file << "\tFluid\n";
                //---------------------------------------------------------------------------------
            }
...
void OUTPUT_MPCNS::process_para_id()
{
    int32_t procnum = pr->GetInt("proc_num");
    ...
    int32_t count_period = 1; // 计算send_flag周期数量的时候选择从1开始
            ...
            for (int32_t iiblock = 0; iiblock < gp->proc_num(myid); iiblock++)
            {
                ...
                for (int32_t jjblock = 0; jjblock < gp->proc_num(tar_myid); jjblock++)
                {
                    ...
                    for (int32_t kface = 0; kface < parallel_id(iblock, jblock).inner.size(); kface++)
                    {
                        ...
                        {
                            // 如果是周期边界条件产生的内部边界条件，flag标记为奇数
                            parallel_id(iblock, jblock).inner[kface].volume = count_period * 2 - 1;
                            count_period++;
                        }
                        else
                        {
                            parallel_id(iblock, jblock).inner[kface].volume = 2 * count;
                            count++;
                        }
                    }
                }
...
void OUTPUT_MPCNS::output_parallel_file(int32_t my_id)
{
    std::ofstream file;
    std::string _my_id_s;
    if (my_id < 10)
    ...
    for (int32_t iiblock = 0; iiblock < gp->pro_block_index(my_id).size(); iiblock++)
        file << "\t" << count_blk[iiblock];
    file << std::endl;
    file << "imin\timax\tjmin\tjmax\tkmin\tkmax\ts_r_id\ts_flag\t\tr_flag\t\tPhysic_Name\n";
                ...
                if (if_multiphy_facecouple) // 如果是多物理场面耦合，严格输出名称
                {
                    for (int32_t iface = 0; iface < inner_info.inner.size(); iface++)
                    {
                        ...
                        file << inner_info.inner[iface].sub[0] << "\t" << inner_info.inner[iface].sup[0] << "\t"
                             ...
                             << "\t\t" << pt->blk(jjblock).hiera[0].blk_name
                             << std::endl;
                    }
                }
                else // 输出"Fluid"
...
void OUTPUT_MPCNS::output_fvbnd_file()
{
    std::ofstream file;

    file.open("./OUTPUT/geometry/boundary_condition/boundary_name.fvbnd", std::ios_base::out);
    file << "FVBND 1 3\n";
    file << "No_Boundary_Condition\n";
    for (int32_t i = 2; i < pt->phy_name.size(); i++)
    {
        file << pt->phy_name[i] << std::endl;
    }
    file << "BOUNDARIES";
    file.close();

    std::cout << "\t\tFinish output the Fvbnd file\n";
}

void OUTPUT_MPCNS::output_gridfile(int32_t my_id)
{
    std::ofstream file;
    ...
    if (grd_writetype == 0)
    {
        file.open("./OUTPUT/geometry/grid" + _my_id_s + ".grd", std::ios_base::out);
        /******输出网格块总数******/
        file << gp->proc_num(my_id) << std::endl;
        /******输出网格块名称******/
        // Remark: 原始ACANS以及plot3D格式的网格文件中没有这一部分
        if (if_multiphy_facecouple) // 严格输出对应的物理场名称
    ...
    {
        file.open("./OUTPUT/geometry/grid" + _my_id_s + ".grd", std::ios::out | std::ios::binary);
        int32_t temp;
        temp = 4;
        ...
        {
            if (if_multiphy_facecouple) // 严格输出对应的物理场名称
                temp_name = pt->blk(gp->pro_block_index(my_id)[i]).hiera[0].blk_name;
            else // 输出Fluid
                temp_name = "Fluid";
            temp_length = temp_name.size() + 1;
            file.write((char *)&temp_length, sizeof(int32_t));
            file.write(temp_name.c_str(), temp_length);
        }
